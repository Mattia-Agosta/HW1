/* Creo la funzione che si occuperà di fare la fetch a getGames.php e
   ...
   - Al submit del form (passando da submitLatestGames)*/
    function searchLatestGames() {
    const search = document.querySelector("#platform");
    //Non è necessario nessun controllo, pocihè di default è selezionata un'opzione
    //Preparo la fetch
    console.log(search.value);
    const search_url = "get_games.php?p=" + encodeURIComponent(search.options[search.selectedIndex].value) ;
    console.log(search_url);
    fetch(search_url).then(onResponse, onError).then(onGamesJson);
}

function submitLatestGames(event) {
...
searchLatestGames();
document.querySelector("form").addEventListener("submit", submitLatestGames);